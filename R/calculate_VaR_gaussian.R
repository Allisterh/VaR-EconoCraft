#' Calculate Gaussian Value at Risk and Expected Shortfall
#'
#' This function calculates the Value at Risk (VaR) and Expected Shortfall (ES) of a portfolio
#' based on historical returns, assuming the returns follow a Gaussian (Normal) distribution.
#'
...
#' \itemize{
#'   \item \code{VaR}: The Gaussian Value at Risk at the specified confidence level.
...
#' alpha <- 0.05
#' calculate_VaR_gaussian(portfolio_returns, alpha)

calculate_VaR_gaussian <- function(portfolio_returns, alpha) {
  stopifnot(alpha >= 0 & alpha <= 1)
  
  mu = suppressWarnings(mean(portfolio_returns))
  sigma = suppressWarnings(sd(portfolio_returns))
